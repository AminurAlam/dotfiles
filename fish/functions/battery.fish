function battery
    ...
    set -f battery (rish -c 'dumpsys battery')
    while printf "$battery" | rg -q 'powered: true'
        printf $battery | rg --only-matching 'current now: .*' | tr -dc [:digit:]
        set battery (rish -c 'dumpsys battery')
    end
end
